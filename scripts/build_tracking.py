...
def build() -> str:
    ...
    lines = [
        ...
        "(**outputs / inputs / aliases**, from the live registry) plus build + quality status. "
        "Not-yet-built indicators show `—` for metadata.",
        "",
    ...
    for category, ids in TARGETS.items():
        ...
        for name in ids:
            ...
            lines.append(
                ...
                f"| {_m(parity)} | {_m(xlib)} | {_m(real)} | {_m(invalid)} | {_m(review)} "
                f"| {status} |"
            )
        lines.append("")
