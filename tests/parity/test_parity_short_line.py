"""Short Line parity — EXACT integer match vs ``talib.CDLSHORTLINE`` (synthetic + real).

``CDLSHORTLINE`` emits only -100/0/100 (no partial ±80 score). Parity is checked with no
...
def _check(df):
    our = INDICATORS.create("short_line").compute(df)["short_line"].to_numpy()
    ref = talib.CDLSHORTLINE(*_ohlc(df)).astype("float64")
    assert ref.shape == our.shape
    np.testing.assert_array_equal(our, ref)


def test_short_line_parity_synthetic():
    _check(deterministic_frame())


def test_short_line_parity_real():
